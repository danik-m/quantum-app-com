...
from curses import A_COLOR, COLORS
...
try:
    from pages.orbitals_page import AUFBAU, parse_orb, draw_orbital_diagram
except ImportError:
...
def draw_frame(config, title=None, figsize=(10,5)):
    ...
    for i, n in enumerate(n_vals):
        for j, col in enumerate(cols):
            ...
            # orbital label (left)
            ax.text(x-1.05, y+0.18, f"{found['orb']}", color=TEXT_COLOR, fontsize=9, weight="bold", va="center")
...
def build_incremental_configs(Z):
    ...
    for orb in AUFBAU:
        n, sub = parse_orb(orb)
        cap = CAPACITY[sub]
        full_cfg_template.append({"orb": orb, "n": n, "subshell": sub, "cap": cap})
    ...
    if len(configs) == 0:
        configs.append([ {**e, "elec": (min(e["cap"], Z) if Z>0 else 0)} for e in full_cfg_template ])
    return configs
...
with right_col:
    ...
    if show:
        ...
        if st.session_state.get('auto_play', True):
            ...
            total_steps = len(AUFBAU_LIST)
            # prepare incremental configs for each step
            incremental_configs = []
            for step in range(total_steps):
                # compute occupancy up to step
                remaining = selected_Z
                conf = []
                for i, orb in enumerate(AUFBAU_LIST):
                    n = int(orb[0]); subshell = orb[1]
                    cap = CAPACITY[subshell]
                    if remaining <= 0:
                        elec = 0
                    else:
                        elec = min(cap, remaining)
                    conf.append({"orbital": orb, "n": n, "subshell": subshell, "electrons": elec, "capacity": cap})
                    remaining -= elec
                incremental_configs.append(conf)
            # play animation quickly by re-rendering diagram with small sleep
            ...
            if st.session_state['anim_running']:
                for frame_idx in range(len(incremental_configs)):
                    fig = draw_orbital_diagram(incremental_configs[frame_idx], title=f"Z={selected_Z}  step {frame_idx+1}/{len(incremental_configs)}", figsize=(12,6))
                ...
                fig = draw_orbital_diagram(cfg, title=f"{selected_Z} {data['symbol']} — Z={selected_Z}", figsize=(12,6))
                diagram_placeholder.pyplot(fig)
        else:
            ...
            for i, orb in enumerate(AUFBAU_LIST):
                ...
                conf.append({"orbital": orb, "n": n, "subshell": subshell, "electrons": elec, "capacity": cap})
                remaining -= elec
            fig = draw_orbital_diagram(conf, title=f"{selected_Z} {data['symbol']} — manual", figsize=(12,6))
            diagram_placeholder.pyplot(fig)
...
def render_periodic_grid():
    ...
    placement_map = {}
    # Build simplified mapping using standard sequence for main-blocks and transition metals
    # This is long; we'll compute approximate positions:
